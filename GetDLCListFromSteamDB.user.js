...
class Main
{
    ...
    CreateModal()
    {
        ...
        this.modal.innerHTML = `<div class="modal">
          ...
          <div class="modal-header">
            <h3>Get DLC List from SteamDB <b>v1.0.15</b> <small>by JNCJcoder</small></h3>
          </div>
    ...
    PopulateDLCList()
    {
        ...
        const dlcTable = appTable
            .filter(dlc => 
                !(dlc.tagName != "TR" || dlc.textContent.includes("SteamDB"))
            )
            .map(dlc =>
            {
                const appID = dlc.childNodes[1].textContent;
                const name = dlc.childNodes[3].textContent;
